...
# We will load a custom dictionary from RAE.and HTML files.
...
def getCorrectedWord(word):
    ...
    if (
        word.lower() in spell_en
        or word.lower() in spell_de
        or word.replace("í", "i").lower() in spell_en
    ):
        return word, word
...
def processText(doc):
    ...
    totalWords = set(
        [word for word in re.findall("\w+", correctedText) if word not in stopwords_es]
    )
    ...
    # FOURTH CORRECTION
    # We eliminate orphan letters.
    correctedText = re.sub(r"\b\w\w?\b", "", correctedText, re.VERBOSE)
    ...
    return correctedText, data


# MAIN FUNCTION
files = sorted(glob.glob("../data/rawPDF/*/*.json"))

# Let's keep track of already checked articles to resume parsing in case
# we need to interrupt.
# TODO: Implement if parsedpdfs.json does not exist.
with open("parsedpdfs.json") as fp:
    checked = json.load(fp)

counter = len(checked)

if counter > 0:
    print(f"Resuming parsing. Starting from {counter} of {len(files)} checked.\n---")
...
for file in files:
    ...
    # We skip if the document is not in Spanish or was already checked.
    if doc["lang"] != "es" or doc["id"] in checked:
        continue
    ...
    The previous implementation used PyPDF2 instead of Ghostcript. However,
    it didn't read some pdfs correctly, so now we use Ghostscript to extract text.
    ...
    # ADD: Let's also only count articles to save time.
    if text.split() and doc["type"] == "ARTÍCULOS":
        doc["text"], doc_data = processText(doc)
    else:
        doc["text"] = None
    ...
    with open("parsedpdfs.json", "w") as fp:
        json.dump(checked, fp)

    with open("parsingpdf_data.csv", "a") as fp:
