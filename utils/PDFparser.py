...
spell.word_frequency.load_dictionary(
    '../notebooks/wordlists/customDictionary.gz')
...
def getCorrectedWord(word):
    ...
    if len(re.findall('\w*filosof[ií]a\w*', word)) > 0:
        correctedWord = re.sub(
            '(\w*)filosof[íi]a(\w*)', r'\1 filosofía \2', word)
        # print(correctedWord)
...
def processText(doc):
    ...
    totalWords = set([word for word in re.findall(
        '\w+', correctedText) if word not in stopwords_es])
    ...
    correctedText = re.sub(
        r'([A-Z]+)[li1]v[li1]([A-Z]+)', r'\1M\1', correctedText)
    ...
    correctedText = re.sub(
        r'\b\w\w?\b', '', correctedText, re.VERBOSE)
    ...
    unknownWords = [word for word in words if word.lower()
                    in spell.unknown(words)]
    ...
    for word, correctedWord in correctedWords:
        correctedText = re.sub(r'\b' + word + r'\b',
                               correctedWord, correctedText)
